...
def runCNN():
    ...
    if os.path.exists('model/model.json'):
        ...
        # Load model weights (Ensure compatibility with TensorFlow 2.x)
        try:
            cnn_model.load_weights("model/model_weights.h5")
        except Exception as e:
            print("Error loading model weights:", e)
            print("Trying to reload weights with the correct format.")

        cnn_model._make_predict_function()  # Ensure prediction function is ready
        print(cnn_model.summary())
        ...
        # Define the model architecture if it doesn't exist
        ...
        # Save model weights and architecture in the new format
        cnn_model.save_weights('model/model_weights.h5')
        model_json = cnn_model.to_json()
        with open("model/model.json", "w") as json_file:
            json_file.write(model_json)
    ...
    # Convert the test labels to 1D array
    y_test = np.argmax(y_test, axis=1)
...
def runCNNSVM():
    ...
    # Get the CNN model predictions
    predict = cnn_model.predict(X)
    
    # Convert predictions to 1D label array
    YY = []
    for i in range(len(predict)):
        val = np.argmax(predict[i])  # Get the class with the highest probability
        YY.append(val)
    YY = np.asarray(YY)
    
    # Ensure that there is more than one class in YY
    unique_classes = np.unique(YY)
    if len(unique_classes) <= 1:
        print("Error: The dataset has only one class. SVM requires at least two classes.")
        return  # Exit the function if there is only one class
    ...
    # Predict with SVM and evaluate performance
    predict = rfc.predict(X_test)
    ...
    text.insert(END, "CNN with SVM Recall    : " + str(r) + "\n")
    text.insert(END, "CNN with SVM FMeasure  : " + str(f) + "\n")
    text.insert(END, "CNN with SVM Accuracy  : " + str(a) + "\n\n")
